...
(defun point-test-motion (function marked-text)
...
(point-test-motion \"Hello wor①l②d!\" 'backward-char) signals an error."
  ...
  (let ((start (point-test--between (point-test--s-remove marked-text "②") "①"))
	...
	(ert-fail (s-join "\n"
			  (list
			   ...
			   (format "function          : %s" (symbol-name function))
			   (format "text              : [%s]" text)
...
(defun point-test-result (marked-text function result)
  "Move the point to the | in the MARKED-TEXT and run FUNCTION.
Verify that it returns RESULT."
  (let ((point (point-test--between marked-text "|"))
...
(defun point-test-edit (start-marked-text end-marked-text function)
  "Move the point to the | in the START-MARKED-TEXT and run FUNCTION.
Verify that it is at | in the END-MARKED-TEXT."
       (let ((start-point (point-test--between start-marked-text "|"))
